...
class User():
    ...
    def __init__(self, first_name, last_name, email,
        ...
        self.profile_pic_url = profile_pic_url if isinstance(
            profile_pic_url, str) else str("/static/imgs/profile-avatar.png")
        self.user_banner_url = user_banner_url if isinstance(
            user_banner_url, str) else str("/static/icons/book.png")
        self.user_member_since = user_member_since
        self.user_city = user_city if isinstance(user_city, str) else str("")
        self.user_location = user_location if isinstance(
            user_location, str) else str("")
        self.user_interests = user_interests if isinstance(
            user_interests, str) else str("")
        self.user_biography = user_biography if isinstance(
            user_biography, str) else str("")
        self.user_project = user_project if isinstance(
            user_project, str) else str("")
        self.groups_member_of = groups_member_of if isinstance(
            groups_member_of, list) else []
        self.groups_created = groups_created if isinstance(
            groups_created, list) else []
